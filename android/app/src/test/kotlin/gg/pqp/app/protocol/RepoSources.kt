...
object RepoSources {
    ...
    /**
     * Every frame `type` discriminant declared anywhere in the shared package.
     *
     * Read from the directory rather than from a list of filenames, because a
     * list of filenames is one more hand-copy and goes stale the first time
     * somebody splits a schema out. It already had: `friend-activity` lives in
     * `friends.ts`, and a hard-coded chat.ts + signaling.ts pair reported the
     * Android client's perfectly good handler as a frame the server never
     * declares.
     */
    fun sharedFrameTypeLiterals(): Set<String> =
