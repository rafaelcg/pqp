...
describe("pickServerLandingTarget", () => {
  it("opens the first text channel when Baú is off", () => {
    expect(pickServerLandingTarget(channels, false, false)).toEqual({
      kind: "channel",
      id: "geral",
    });
  });
  ...
  it("falls back to any non-category channel, and to nothing at all", () => {
    expect(
      pickServerLandingTarget([{ id: "lobby", type: "voice" }], false, false),
    ).toEqual({ kind: "channel", id: "lobby" });
    expect(
      pickServerLandingTarget([{ id: "cat", type: "category" }], false, false),
    ).toBeNull();
  });
});
