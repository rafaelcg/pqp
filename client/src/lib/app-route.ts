...
export type AppRouteTarget =
  | { kind: "channel"; serverId: string; channelId: string | null }
...
export function parseAppRoute(pathname: string): AppRouteTarget | null {
  ...
  const [, section, first, nested, second] = segments;
  ...
  if (section === "server" && first) {
    return {
      ...
      channelId: nested === "channel" && second ? decodeURIComponent(second) : null,
    };
  }
