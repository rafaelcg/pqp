...
/**
 * LiveKit's default sliding playlist is about five 2 s segments = 10 s.
 * `SegmentedFileOutput` in livekit-server-sdk 2.17.0 has no `playlistLength`,
 * so this window is not ours to widen; sit inside it instead of on its back.
 *
 * `liveSyncDurationCount: 4` / `liveMaxLatencyDurationCount: 5` parked the
 * playhead on the oldest of those five segments — the one the next playlist
 * update expires. A stall then had nowhere to go. Sit 2–3 segments behind
 * live (6 s) and skip forward at 4 segments (8 s), which is still strictly
 * inside the 10 s playlist, not equal to it.
 */
export const HLS_LIVE_SEGMENT_SECONDS = 2;
export const HLS_LIVE_WINDOW_SECONDS = 10;
/** ~6 s behind live: the middle of the 5-segment window, not the oldest. */
export const HLS_LIVE_SYNC_DURATION_COUNT = 3;
/**
 * Skip forward before falling off the back. Must be greater than the sync
 * count and strictly less than the playlist (5 × 2 s), or a seek-to-live
 * lands on the segment that is about to disappear.
 */
export const HLS_LIVE_MAX_LATENCY_DURATION_COUNT = 4;
/** Match the 6 s of listed content in front of a 3-segment sync; do not ask for the whole playlist. */
export const HLS_MAX_BUFFER_LENGTH_SECONDS = 6;
export const HLS_MAX_MAX_BUFFER_LENGTH_SECONDS = 8;
 ...
 * window. The attach path must not start on the oldest listed segment (the
 * one the next playlist update expires) or set max-latency equal to the
 * whole playlist.
 */
export function hlsLiveSyncFitsWindow(config: HlsLivePlayerConfig): boolean {
  ...
  return (
    // Oldest segment occupies the last 2 s of the window. Sitting there
    // (`liveSyncDurationCount: 4` of 5) is the stall this helper exists to
    // refuse.
    sync < HLS_LIVE_WINDOW_SECONDS - HLS_LIVE_SEGMENT_SECONDS &&
