...
describe("an LL player that falls behind the part ring", () => {
  ...
  it("is bounded: past the budget the error escalates the way it always did", async () => {
    // On the conventional path, where the LL pin rule is not also watching:
    // the jump budget is the only thing bounding this, which is the point.
    await mount("live");
    for (let i = 0; i < LL_HLS_EDGE_JUMP_MAX; i += 1) {
      await act(async () => {
        fire(missingPartError());
      });
      // One segment back, not one part: `liveSeekOffsetSeconds`.
      expect(calls).toContain("startLoad(1496)");
      calls.length = 0;
    }
    await act(async () => {
      fire(missingPartError());
