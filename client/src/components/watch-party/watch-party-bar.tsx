...
export function WatchPartyBarSlot({
  ...
  return (
    <div
      ...
      className={cn(
        placement === "stage"
          ? // OVER THE BOTTOM EDGE OF THE PICTURE, chrome layer above the stage's own
            ...
            `pointer-events-none absolute inset-x-0 bottom-0 ${STAGE_LAYER.chrome} flex flex-wrap items-center gap-1.5 bg-gradient-to-t from-ink/95 via-ink/70 to-transparent px-3 pb-2 pt-6 empty:hidden [&>*]:pointer-events-auto`
          : placement === "status"
            ? // THE STATUS LINE OVER THE TOP EDGE OF THE PICTURE (pass 3): the
              ...
              `pointer-events-none absolute inset-x-0 top-0 ${STAGE_LAYER.chrome} flex items-start px-3 pt-2 empty:hidden [&>*]:pointer-events-auto`
