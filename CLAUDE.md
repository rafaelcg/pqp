 ...
 *not* scanned, and the CSAM reporting runbook; `docs/PWA.md` for the mobile/installable app; `docs/ANDROID.md` for the native Android client, why it is native rather than a TWA, what is actually verified, and what the wire-contract tests in `android/app/src/test` pin against `packages/shared`; `docs/ANDROID_RELEASE.md` for the Play closed beta (upload keystore, foreground-service justification, data safety, pt-BR listing) and the in-app account deletion that blocks it; `docs/TESTFLIGHT.md` for the iOS beta / App Review demo account; `tools/admin-dashboard/README.md` for the operator dashboard (`pqp-admin` Worker) and its `ADMIN_METRICS_TOKEN` / Basic Auth secrets.
