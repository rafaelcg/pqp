...
| Beta join links (public, never secrets) | `VITE_TESTFLIGHT_URL` (iOS; a public default lives in `client/src/lib/testflight.ts`, so this only overrides it). `VITE_ANDROID_APK_URL` (Android APK; empty uses GitHub `releases/latest/download/pqp.apk`, a single space hides the button). The join surfaces are `/beta` and `/android`. |
