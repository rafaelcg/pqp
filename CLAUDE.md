...
| Client | `VITE_CLERK_PUBLISHABLE_KEY`, `VITE_API_URL`, `VITE_WS_URL`, `VITE_DEV_AUTH_BYPASS`, `VITE_VOICE_BACKEND` (leave empty to follow the server; `mesh` forces peer-to-peer), `VITE_COMMUNITY_HOME_ENABLED` (client-only Home feed mock; default off; do **not** reuse `COMMUNITIES_ENABLED`; see `docs/COMMUNITY_HOME.md`) |
