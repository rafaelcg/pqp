...
7. Say `restarts-api` in the PR when a server or `packages/shared` change will redeploy `pqp-api`. Fly closes every `/ws`. After #162, refreshed web and Electron keep the media session and resume the same peer id. iOS, Android, failed resume, and tabs that have not refreshed still drop out of voice. Client-only Pages deploys do not restart the API. This is the usual case. It is not a hangup. Apply `drops-voice` only when the change itself will hang up a live call even after resume (breaking resume, peer-id rules, or the media path). An API restart alone does not earn that label. CI stamps `size/XS`–`size/XL` from the diff; leave those alone.
