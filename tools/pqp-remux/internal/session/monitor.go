...
func (s *Session) Stats() Stats {
	...
	st := Stats{
		...
		VideoLatePackets:     s.videoLatePackets.Load(),
...
func formatStatsLine(label string, prev, cur Stats, window time.Duration) string {
	...
	fmt.Fprintf(&b, " | video pkts=+%d (%.1f/s) frames=+%d (%.1f/s) idr=+%d drops=+%d lost=+%d late=+%d damage=+%d damagedDropped=+%d parts=+%d (%.1f/s) segs=+%d keepalive=+%d idle=%t",
