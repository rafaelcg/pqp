...
// in isolation: a session whose fake pipeline never reports any progress
// must be restarted once and then demoted, entirely on its own, within a
// few real ticks -- this is this task's own acceptance bar ("stopping the
// remux container mid-party demotes inside 5s with no player rebuild")
// exercised end to end with a fake pipeline clock standing in for a real
...
func TestManagedSession_WatchdogRestartsThenDemotes_Integration(t *testing.T) {
	...
	// of ticks; a wide DemoteWindowMs so the second stall is unambiguously
	// "within the window" regardless of exact tick timing.
	wd := WatchdogConfig{PartStuckMs: 50, DemoteWindowMs: 60_000}
	reg := NewRegistry(spy.factory(), GlobalConfig{}, wd, nil)
	...
	// Never touch health: the first pipeline is permanently "stuck" from
	// the moment it's built, so the watchdog restarts it once, and the
	// brand-new pipeline (also never touched) stalls again and gets
	// demoted.
