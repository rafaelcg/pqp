...
| Watch party / cinema mode | full: immersive stage, idle chrome (`client/src/hooks/use-immersive-stage.ts`); tab-share-with-sound control in open PR #258 | same, minus tab share (see send row) | missing: `watch-party`, `voice-stream` and `channel-live` have no hits in `ios/`, so the frames the server sends after `welcome` are dropped as `.other` and somebody who joins a watch party channel simply does not get the film. A feature to build rather than a bug to fix | missing (no hits in `android/`) |
