...
| Capability | Web | Electron | iOS | Android |
|---|---|---|---|---|
| Voice channel row: tap opens the view, does not join | full: single click/tap selects and shows chat + stage, double click or a second tap joins, Enter joins, the Entrar button and header join button still join (`client/src/components/layout/channel-list.tsx` `ChannelRow`); matches #339 (Android/iOS) | same | full since #339 (`ios/`) | full since #339 (`android/app/src/main/kotlin/gg/pqp/app/ui/screens/ChannelsScreen.kt`) |
