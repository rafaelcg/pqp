...
| Follow a mid-call room promotion | full: keeps the seat and the peer id, republishes mic / camera / share (`client/src/hooks/use-voice.ts` `voice-transport-changed`) | same | full since #410: declares `voice-transport-changed`, keeps the seat and the peer id, and restarts a running camera or share on the SFU (`ios/Voice/VoiceModel.swift` `followPromotion`, `ios/Voice/VoiceTransport.swift`). **Unverified on hardware** | full for audio: declares the capability, stops its mesh share, swaps engines against the same peer id (`android/voice/TransportChange.kt`, `android/voice/VoiceController.kt` `onTransportChanged`). Audio cuts for the token mint plus the SFU handshake. **Unverified on hardware** |
| Resume media across an API restart | full, 90 s orphan window (`client/src/lib/realtime.ts`, `client/src/hooks/use-voice.ts`) | same, plus `setBackgroundThrottling(false)` (`electron/main.js:939`) | partial: LiveKit resumes, mesh is rebuilt. Since the ghost-seat PR the app only ASKS for a held seat where it can honour it (`declaresVoiceResume` in `ios/Voice/VoiceTransport.swift`); it used to ask on every room on a LiveKit deployment, which left a phantom participant in every mesh call's roster for 90 s. One gap remains: a cold join to a **server** voice channel cannot know its transport yet, so a small server's mesh channel can still ghost once per session | partial: call is rebuilt, not resumed; `resumeToken` sent since PR #270 (`android/voice/VoiceController.kt`) |
...
| Push-to-talk | full, in-window only, mouse-button binding via `mousedown`/`mouseup` (`client/src/components/voice/use-push-to-talk.ts`) | full: a native global keyboard/mouse hook (`uiohook-napi`) while the window is unfocused, in-window otherwise, with a configurable release delay (default 20 ms) and mouse-button binding, backed by the old `globalShortcut` inference as a fallback when the hook is unavailable, denied, or not built (`electron/lib/native-ptt-hook.js`, `electron/lib/global-ptt.js`, `electron/main.js`); macOS needs the Input Monitoring/Accessibility permission and shows an in-app nudge when denied; Linux/Wayland has no native-capture backend at all and falls back to in-window plus the Voice Activity mode instead; menu has Cmd/Ctrl+Shift+M toggle mute | missing (`ios/Core/APIClient.swift:546` says `inputMode` is not modelled) | missing |
| Global mute/deafen hotkeys | in-window only (the Settings shortcut table, `client/src/lib/keyboard-shortcuts.ts`) | full: the current toggle-mute / toggle-deafen bindings also work while the window is unfocused, held only during a call (`client/src/lib/global-voice-hotkeys.ts`, `client/src/App.tsx`, `electron/main.js` `syncGlobalVoiceHotkeys` / `setGlobalVoiceHotkeys`) | missing | missing |
...
| Server mute / deafen / disconnect / move (moderator) | full on LiveKit (`client/src/components/layout/members-panel.tsx`); mesh in open PR #223 | same | missing, and **silently** so: `voice-moderation` is not merely unhandled, it is UNDECODABLE. `Envelope.message` in `ios/Core/RealtimeClient.swift` is typed `Message?` and the frame's `message` is a `String`, so the shared envelope throws and the `try?` around it swallows the whole frame. A mod disconnect therefore ends the call with no reason given, and a mod **move** drops the person while everyone else watches them arrive somewhere else. Needs its own `Decodable`, the way `SanctionFrame` and `PresenceDeltaFrame` already do. Receiver side in open PR #222 | full receiver side: `voice-moderation` muted / unmuted / moved / disconnected (`android/voice/VoiceController.kt`); mesh in open PR #221; no moderator UI |
| SPEAK / STREAM permission enforcement | full: listen-only join, disabled unmute, badge (`client/src/components/voice/capabilities.ts`); STREAM split in open PR #254 | same | full for SPEAK: `welcome.canSpeak` and `voice-speak-changed` lock the mic and hide share, with the web's copy (`ios/Voice/VoiceSpeakRule.swift`, `ios/Voice/VoiceView.swift:275`). STREAM split since the ghost-seat PR: `canStream` is carried on the participant, on `welcome` and on `voice-speak-changed`, gates the camera and share controls, and takes a running capture down on a revoke (`ios/Voice/VoiceSpeakRule.swift`, `ios/Voice/VoiceModel.swift`). Absent on the wire follows SPEAK, never defaults open | full for SPEAK (`android/voice/SpeakRule.kt`, `android/voice/VoiceController.kt:741`, `android/ui/components/CallBar.kt:147`); STREAM not split yet |
...
| Watch party / cinema mode | full: immersive stage, idle chrome (`client/src/hooks/use-immersive-stage.ts`); tab-share-with-sound control in open PR #258 | same, minus tab share (see send row) | partial: the AUDIENCE half is built. `voice-stream` and `channel-live` decode, `watch-live` subscribes without a seat, and `AVPlayer` plays the `?t=` playlist above the transcript with background audio and lock screen controls (`ios/Voice/WatchStageView.swift`, `ios/Voice/WatchModel.swift`, `ios/Voice/LiveStream.swift`). Still missing: the party OBJECT (`watch-party-update`, host, cohosts, stage, raise hand) and the presenter side, so an iPhone can watch a watch party but cannot run one. **Unverified on hardware and against a live egress** | missing (no hits in `android/`) |
...
| Global push-to-talk hotkey | full: a native keyboard/mouse hook while the window is unfocused (real key/button down-up, configurable release delay, mouse-button binding, macOS permission nudge), falling back to `globalShortcut` (keyboard only, release inferred from auto-repeat) when the hook is unavailable, denied, or the shell predates it; no native-capture backend at all under Linux/Wayland | `electron/lib/native-ptt-hook.js`, `electron/lib/global-ptt.js`, `electron/main.js` |
| Global mute/deafen hotkeys | full: the current toggle-mute / toggle-deafen chords, registered while the window is unfocused and only during a call | `client/src/lib/global-voice-hotkeys.ts`, `electron/main.js` `syncGlobalVoiceHotkeys` |
...
- `ios/Voice/CallRating.swift` claimed the app declares `transports: ["mesh"]`
  and cannot produce a `livekit` rating. That was true for about a week in build
  16 and had been read as current ever since. Corrected in the ghost-seat PR;
  the app has declared both transports since LiveKit joining landed.
