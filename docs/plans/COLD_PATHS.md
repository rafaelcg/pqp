...
| Voice ghost-seat sweep (`sweepOwnStaleVoicePeers`) | 15 s, on the instance heartbeat | one DELETE keyed on this instance's own un-orphaned rows | **Yes**: the peer ids it spares come from this process's map, and nothing else can supply them | `ws/voice.ts` (`runVoiceReconcile`) | **Stays**. It is the one sweep that is *about* the difference between the rows and an in-process map, so it can only run where that map is |
...
| WS heartbeat (ping/pong, reap dead sockets) | 30 s | walks `wss.clients` | **Yes** | `ws/index.ts` (`startHeartbeat`), called from `index.ts` | **Stays**, obviously. One implementation: until 2026-09-08 `index.ts` had an inline copy and the tested one in `ws/index.ts` never ran |
