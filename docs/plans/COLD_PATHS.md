...
| Voice ghost-seat sweep (`sweepOwnStaleVoicePeers`) | 15 s, on the instance heartbeat | one DELETE keyed on this instance's own rows | **Yes**: the peer ids it spares come from this process's map, and nothing else can supply them | `ws/voice.ts` (`runVoiceReconcile`) | **Stays**. It is the one sweep that is *about* the difference between the rows and an in-process map, so it can only run where that map is |
