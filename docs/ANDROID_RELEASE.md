...
| `assembleRelease` no longer falls back to the debug key | **Done** — with no keystore it now produces an *unsigned* APK instead of a plausible-looking rejected one |
...
| **In-app account deletion** | **MISSING. This is a launch blocker.** See §0 |
| Data export in the app | Missing (Play does not require it; Brazil's LGPD arguably does) |
| Play Console: FGS declaration, data safety, listing | **Not started.** §5, §6, §7 |
| Voice carrying actual audio | **Unproven.** See `ANDROID_PLAN.md` A7 — decide §3 before uploading |

---

## 0. The blocker to fix before anything else

**The Android app has no way to delete an account.** `YouScreen.kt` offers
exactly one action, Sign out. There is no `DELETE /api/me` call anywhere in
`android/app/src/main/kotlin`, and no data export either.

This is not a nice-to-have. Play's User Data policy requires an app with
accounts to offer in-app deletion **and** a web deletion route, and it is the
exact thing that held the iOS build back (see `HANDOVER.md`, build 12). Finding
it again at upload would be paying twice for one lesson.

The server side already exists and needs nothing:

- `DELETE /api/me` — `server/src/api/index.ts:1233`. Body is
  `{ "confirm": "<name#1234>" }`; the fallback phrase for an account with no tag
  is `delete my account` (`DELETE_ACCOUNT_FALLBACK_PHRASE` in
  `packages/shared/src/api.ts`). Rate limited, and it refuses character
  accounts.
- `GET /api/me/export` — `server/src/api/index.ts:1187`.
- The web already implements both in
  `client/src/components/layout/settings-modal.tsx`, which is the reference to
  copy the confirmation semantics from.

**Owner:** the integration agent, not this one. It is app feature code
(`YouScreen.kt`, `ApiClient.kt`, `strings.xml` in both languages).
...
- **A7 lands** — the two-device runbook passes with real TURN credentials on the
...
the Console or the release is rejected. `AndroidManifest.xml` declares exactly
one today:
...
When B2 (screen sharing) lands, `mediaProjection` gets added to the manifest and
**this form has to be filled in again for it**, with its own video. A new
foreground service type in a release with no matching declaration is a rejection.
...
| Can users request deletion? | **Yes** — in-app, and a web route. *This answer requires §0 to be done first.* |
