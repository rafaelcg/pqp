...
- **Character HTTP send (PR, not on `main`):** `POST /api/channels/:channelId/messages` with `Bearer character:<token>`. Same create + fan-out as the WebSocket `message-create` frame (history, notify, thread replies, outgoing-webhook skip for `is_character`). Clerk sessions get 403. Mint tokens with the existing provision scripts; there is no admin UI. Merging **restarts `pqp-api`**. [`BOT_SEND.md`](./BOT_SEND.md).
