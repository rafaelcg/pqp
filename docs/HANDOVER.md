...
- **Voice session resume (web / Electron):** an API restart closes `/ws` with 1001. Web and Electron keep the mesh or LiveKit session and rejoin with the same peer id (HMAC token, 90s orphan TTL). iOS and Android still cold-join. Closing a tab sends `leave-voice-room` on `pagehide` so it is not an orphan. Electron turns off background throttling on the main window so a minimized call can still resume. Merging this **restarts `pqp-api`** and drops live voice once (old JS). After a refresh, later API deploys should not cut web/Electron audio.
