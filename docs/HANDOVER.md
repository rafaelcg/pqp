...
- **Two `pqp-api` machines, prepared not flipped (2026-09-14):** `fly.toml` is pinned for two Fly machines with rolling deploys (`performance-1x`/2048mb, `WORKER_MODE=api`, `CLUSTER_BUS`/`VOICE_REGISTRY` moved from secrets into `[env]`, `PG_POOL_MAX` recomputed for the current Vultr-managed database), `deploy-api-fly.yml`'s post-deploy check now verifies `/health` on every started machine individually via `fly-force-instance-id` rather than trusting the load-balanced hostname, and `docs/deploy-fly.md` §6a-bis carries the exact flip order, a shadow-secret trap worth reading before running it, rollback, and how to scale past two. Nothing in production changed yet — see the PR for the open prerequisites (age-gate cache fix, cross-instance cache audit, attachments/watch-party-start delivery fixes, a green staging matrix rerun) before the flip, targeted for a quiet window ahead of the 2026-09-19 party. Background: `docs/plans/M6_REHEARSAL_2026-09-13.md` and `docs/plans/M6_REHEARSAL_2026-09-14.md`.
