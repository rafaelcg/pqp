...
- **Community Home mock (client-only, flag off by default):** Patreon-like durable feed behind `VITE_COMMUNITY_HOME_ENABLED` / `pqp:community-home` / `?communityHome=1`. Pinned Home/Início above TEXT, default land on the feed, free vs VIP visibility using existing `system_key=vip`, viewer flip via header or `?homeViewer=`. Fixtures in localStorage. **No schema, no API, no Fly restart.** Do not turn on in production. See [`COMMUNITY_HOME.md`](./COMMUNITY_HOME.md).
