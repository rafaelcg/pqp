...
- **In-app profile card (2026-08-28):** restyled as a contact sheet. Equal action tiles, rank marks (crown / shield) beside the name, cargos without the seeded Admin pill, and an about pane (Depoimentos / Contas / Comunidades; Contas first when connections exist). Turma and caça-bugs render as a medal and a pin, not chips. Member list search is in the same pass. Client-only: Pages deploy, no API restart.
