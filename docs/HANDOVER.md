...
- **Community Home (Baú / Home, flag off by default):** Durable media library behind `VITE_COMMUNITY_HOME_ENABLED` / latch. Per-server toggle in Server Settings (MANAGE_SERVER, default OFF). Postgres posts/comments/likes + R2 media on staging API. pt-BR **Baú**, EN **Home**. No localStorage posts, no seed feed, no Compose|Preview chrome. Do not turn on in production Pages. Staging-only until promoted. See [`COMMUNITY_HOME.md`](./COMMUNITY_HOME.md).
