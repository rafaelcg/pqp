...
- **Copy Discord layout (2026-08-25):** paste a Guild Template link (`discord.new/…`) when creating a community. Preview names the private channels and what will be dropped, then one transaction creates the pqp server. Cosmetic roles only. No Discord OAuth, no bot, no members or messages. See [`DISCORD_IMPORT.md`](./DISCORD_IMPORT.md).
