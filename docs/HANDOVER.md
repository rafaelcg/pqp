...
- **Community Home mock (client-only, flag off by default):** Patreon-like durable **media** feed behind `VITE_COMMUNITY_HOME_ENABLED` / `pqp:community-home` / `?communityHome=1`. Pinned Home/Início above TEXT on **community** servers, default land on the feed, free vs VIP visibility using existing `system_key=vip`, staff CMS Compose|Preview (manage-server publishes; VIP cannot), flat comments, fixtures in `pqp:community-home-posts:{serverId}`. **No call CTA, no schema, no API, no Fly restart.** Do not turn on in production. See [`COMMUNITY_HOME.md`](./COMMUNITY_HOME.md).
