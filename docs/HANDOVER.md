...
**Multi-instance rehearsal (M6, `pqp-api` still one machine in production).** Two-machine signaling-only rehearsal 2026-09-13, three-machine rehearsal with real LiveKit media, a rolling deploy under load and a 3→2→3 scale test 2026-09-14: [`plans/M6_REHEARSAL_2026-09-14.md`](./plans/M6_REHEARSAL_2026-09-14.md). Closed: rings across instances, moderator mute against real published media, the stale `ADMIN_METRICS_TOKEN` blocker, `deploy-staging.yml`'s rolling strategy exercised at three machines. A same-instant multi-machine boot was found to flap Postgres's circuit breaker on a small database (every unset-`WORKER_MODE` machine runs every batch sweep on its own timer, so N machines means N× the sweep query rate in synchronized bursts) — `WORKER_MODE=api` on `pqp-api` is now a prerequisite for the production flip, not a follow-up, since `pqp-worker` already exists. Two findings (cross-instance delivery of an attachment message and of watch-party state to a third socket) were not root-caused and are not yet filed as defects. See the doc's "Blockers before production" before scaling `pqp-api` past one machine.

**How many people fit in one voice room, measured (2026-09-07).** Measured on staging sized like production, three times against shipping `main`, as the occupancy at which joins stop fitting the client's own 12 second budget. **The figure is not in this repository**: it is in the operator's copy at `~/.config/pqp/capacity-measured.md`, for the reason set out at the top of [`CAPACITY.md`](./CAPACITY.md). What is worth carrying here is the shape. What runs out is **bandwidth**, not CPU (which stayed in the thirties as a percentage of two cores) and not the Postgres pool (saturated seconds at a time, behind the wire rather than ahead of it): voice signalling fan-out is O(room size) per arrival, and `presence-update` is now the largest thing on the wire at roughly 70 % of the bytes. The roster delta work bought about 1.4x. Any figure from this rig is a floor, because the harness concentrates every simulated client on one link, and it predates the WebSocket compression and audience roster work that was in flight when it was taken. Method, safety rules and how to run it again: [`STAGING.md`](./STAGING.md) under "Load testing staging"; the media rig and its method: [`CAPACITY.md`](./CAPACITY.md).
...
- **Past broadcasts can be downloaded (2026-09-14):** each available row of "Transmissões anteriores" gets a **Baixar** panel with up to three files: the film (the top ladder rung's MPEG-TS segments, concatenated in playlist order), the presenter's camera pip, and the host's voice archive (only when `LIVE_HLS_MIC_ARCHIVE` was on for that broadcast). Nothing is transcoded and nothing is buffered: the API signs the objects the egress already wrote and pipes them through with backpressure, so a three-hour broadcast never enters the API process's memory. Sizes are a bucket listing per file, asked per broadcast when the panel opens rather than on every history load, and cached for five minutes. `GET .../watch-party/history/:sessionAt/download` lists what exists, `GET .../download/:kind` (`film | camera | voice`) is the bytes, both behind the same permission and the same availability rule as replay (404 unknown kind, 409 once swept). The link is a plain `<a href download>` carrying the `?t=` capability, so the byte route has the same headerless door the replay proxy has. Merging **restarts `pqp-api`**. See [`WATCH_PARTY.md`](./WATCH_PARTY.md) §"Baixar: the film, the camera and the voice".
- **Two `pqp-api` machines, executed live 2026-09-14 13:46–13:49 UTC:** blue-green flip via `fly machine clone` (`docs/deploy-fly.md` §6a-bis) — cloned two `performance-1x`/2048mb machines, verified each individually (`/health`, `/ready`, `runtime.pool.max`, the `[role]` log line, `voice.cluster.*` counters, one real WS connect, all via `fly-force-instance-id`), then stopped the old `performance-2x`/4gb machine: its drain moved 34 sockets onto the new pair with zero proxy gap. `WORKER_MODE`, `CLUSTER_BUS`, `VOICE_REGISTRY` and `PG_POOL_MAX=70` were already live Fly secrets, so both clones inherited role `api` and the rest automatically — no per-machine env step was needed. `PQP_API_MACHINES=2` is set live; this PR (ops/two-machines-fly) must merge before the next `pqp-api` deploy so `fly.toml` (`min_machines_running=2`, the same four names moved into `[env]`, `PG_POOL_MAX=70`) matches the shape already running, or the next ordinary merge diffs a stale file against two machines it didn't create. `deploy-api-fly.yml`'s post-deploy check now verifies `/health` on every started machine individually via `fly-force-instance-id` rather than trusting the load-balanced hostname. Background: `docs/plans/M6_REHEARSAL_2026-09-13.md` and `docs/plans/M6_REHEARSAL_2026-09-14.md`.
- **The camera divides the room too (2026-09-08):** `retuneAllCameraSenders` gave every peer the same `cameraMaxBitrate`, undivided, so a mesh four-way call permitted about 4.46 Mbps of camera off one machine — measured and recorded on 25 Aug and left alone since, while the screen path divided the room from the day it was written and, after #340, divides a *measured* budget. The argument for leaving it undivided was that dividing a chosen "720p" makes the setting mean something different in every call. That is true of a label, and the label is not what is divided: `cameraProfileFor` still pins the capture size, so 720p stays 720p and what shrinks is what the encoder may spend on it. Not dividing did not keep the promise either, it moved the breakage, because four copies of a 1.5 Mbps camera do not fit down a link that carries two and the encoder then misses the ceiling on all four. `meshCameraBitrate` is the twin of `meshScreenBitrate`. **The two senders also stop double-committing the link:** they ride one uplink, `availableOutgoingBitrate` measures that whole uplink, and handing it all to the screen while the camera bid separately is the state `tuneCameraSender`'s own comment described ("the two video senders then bid against each other for one bandwidth estimate with nothing arbitrating, which is how a camera ends up at 240p while the share looks fine"). The room's share is now split between them in proportion to what each was asked for, so a 720p camera (1.5 Mbps) stays modest beside an Auto screen (3 Mbps). The camera follows every move of the measured budget, both edges of a share, and every change of room size. **Measured (2026-09-08).** The harness learned `WITH_CAMERA=true`, which turns the sharer's camera on before the share so a run exercises two senders on one uplink. **Both orderings are measured** (`CAMERA_ORDER=before|after`), because the one that broke was camera-on-during-a-share: nothing retuned the screen on that edge, so it kept the whole share and the pair asked for 133 % of it, and camera-first is the one ordering that cannot show it. Shaped to 3 Mbit the pair settles at screen 783 + camera 391 = 1174 kbps a copy in the `after` run and screen 987 in the `before` one. They do not land on identical numbers and should not be expected to: the budget is measured live off a noisy link. What holds is the shape — the ratio is 2:1 (783/391 = 2.00), the proportion of their chosen ladders, and the pair times the viewers fits inside the link instead of exceeding it. The 2:1 ratio is the proportion of their chosen ladders (Auto screen 3 Mbps, Auto camera 1.5). Before this the camera took its full chosen ceiling per viewer regardless of the room, so the same 3 Mbit run would have asked for about 2994 kbps a copy, nearly double the link. Two harness lessons worth keeping: an env var exported beside `node run.mjs` never reaches the agents, which run in containers with their own environment (the mode now travels in `room.json`, after a first attempt silently ran with no camera and looked like a pass), and `docker compose ... --force-recreate` is unreliable here — do a clean `down -v` then `up -d`. **Web mesh only**, no API change, does not restart `pqp-api`.
- **"Sua conexão está segurando esta transmissão" (2026-09-08):** the app could already tell a weak uplink from a chosen rung (`describeLimitation`, 25 Aug) and kept the answer in a drawer: `OutboundVideoReadout` prints it, but only inside the video quality menu and Settings. Nobody watching a bad share opens a menu; they open #geral and say the app is bad. The sentence now says itself, on the share's own status line, to the presenter only. **Presenter only is a decision, not a gap:** a viewer's machine cannot see the sender's limitation (`VideoReceiverSample` has no counterpart, deliberately), so telling a viewer "your connection is bad" when the presenter's uplink is the cause would be false in the direction that reads as an app deflecting blame. Naming the presenter to the room needs their limitation on the wire, which is a roster field and a server change: worth doing, not done here. Two things had to be right for it to be honest, and running the bandwidth harness is what found both. **One:** once the budget controller adapts, the encoder is handed a ceiling it can meet and reports `qualityLimitationReason: "none"`, so a streak built on that alone reset every few seconds and the sentence was never said, with the adaptation working being the very thing that silenced the explanation for it. The durable signal is the ceiling itself sitting under what was asked for. **Two:** that ceiling is also shrunk by the room, because a mesh sends one copy per viewer, so the rule compares against what the *starting* budget would have allowed for this many viewers, and a five-way call on fibre stays silent. It also fixes a misattribution this same PR would otherwise have introduced: with the budget now moving the ceiling, `describeLimitation` called a budget-imposed ceiling "your quality setting", so the readout would have told somebody on a strained link that their own preference was the limit (`describeLimitationAgainst` is the correction, used by both surfaces). Fires after five consecutive 2 s samples, about ten seconds, so a ramp-up or a re-probe never trips it. Web + Electron, client-only, does not restart `pqp-api`.
- **The mesh screen budget follows the measured uplink (2026-09-07):** `meshScreenBitrate` split a **constant** 5 Mbps across the room, a copy per viewer, and never asked the link what it had. Wrong in both directions: a 3 Mbps uplink sharing to two viewers was granted 2 x 2.5 Mbps, both connections' estimators backed off on their own and fought over the same pipe; fibre sharing to four got 1.25 Mbps a copy with the link barely touched. **This is for rooms of 3 to 8, and that is now enforced rather than merely asserted.** A two-person call has one connection, nothing to split, and Chrome's own congestion control already governs it far better than a 2 s poll can. The first cut of this change *claimed* that in a comment and did not do it: the sampler only skipped an empty room, so a 1:1 call had its ceiling clamped to a measured dip and could then climb back only at 1.3x per tick, undoing recovery the browser would have managed in a second. Caught by an independent review pass, fixed in both places (`nextScreenUploadBudget` declines under two samples; the manager skips sampling and *releases* any clamp left over when a crowded room empties back to one viewer). The 29 Aug QG report ("144p e uns 5 fps", two people) is *not* this bug and still needs the DM diagnosis Raf offered. `client/src/lib/screen-upload-budget.ts` is the fix, an AIMD controller over the room: while a share is up the manager reads `availableOutgoingBitrate` off every peer's selected candidate pair every 2 s (`sampleScreenBudget`), **cuts at once** when the readings say the link is under 90 % of the budget (floor 1 Mbps), and **raises by 1.3x per tick only when every peer** reports more than 1.25x its share (cap 16 Mbps). The asymmetry is deliberate: each estimator only probes a little past what it is sending, so on an unsaturated link the readings under-report and are only a licence to try more, never a reading of capacity. **How the cut reads the paths went through three models, two of them wrong and both caught by review:** `availableOutgoingBitrate` is per-path, bounded by whichever end of that path is narrowest, so the plain **sum** read one 50 kbps viewer as a 50 kbps uplink and dragged a fibre room to its floor in three ticks, punishing the healthy viewer for somebody else's wifi; and the **widest path** alone read one good viewer as a wide uplink and, since every reading is bounded by the ceiling we set, tracked its own past output and sat 2.8x over a 1 Mbps link forever. What ships drops paths narrower than a third of the widest as independently bottlenecked (that viewer's downlink, or a relay on that pair) and takes the mean of the rest times the room — with the guard that outliers must be a minority, since a link everybody divides unevenly is not six outliers and one good path. A peer that cannot be read is assumed to look like the ones that could. The default is unchanged, so the first seconds of a share behave exactly as before, and the budget resets when a share ends (a laptop that moved networks between shares is the ordinary case). The chosen rung and the 4 Mbps per-sender ceiling apply on top exactly as they did; the 600 kbps per-copy floor is gone (see below). **Web mesh only**, no API change, does not restart `pqp-api`. Pinned by `screen-upload-budget.test.ts` and the last `describe` in `peer-connection-tuning.test.ts` (fake timers, fake `getStats`). **Measured on a real shaped link (2026-09-08):** `client/e2e/bandwidth/` is a Docker harness — three real Chromium processes in three real containers, a real mesh call, one container's real upload capped by the Linux kernel (`tc netem`) rather than a mocked stats object. A 3-person call shaped to 3 Mbit mid-share drops the per-copy ceiling to **1494 kbps** and holds, and at 1 Mbit to **500 kbps**; two copies of each is 2988 and 1000 kbps, which is the link in both cases. The harness now also prints the per-path `availableOutgoingBitrate` the controller reads, because the ceiling alone cannot say *why* it chose what it chose, and that is the question two rejected models turned on: at 3 Mbit the paths read `[1729, 1608]`, the same order as each other, so both count as sharing one link rather than one being dropped. Manual, on-demand only (not in CI): see its README for why and for two Chromium gotchas worth not rediscovering. **That harness immediately found a weak-link bug in this same change, which matters because most of this app's users are Brazilian and the weak tail is who this is for.** A 600 kbps *per-copy* floor sat under `meshScreenBitrate`. Against the old constant budget it never fired (5 Mbps across the seven remote peers a mesh can hold is 714 kbps, already above it), so making the budget a measurement woke it up in exactly the case it gets wrong: a room cut to the 1 Mbps minimum floored each of seven copies back to 600 kbps and asked a *measured* 1 Mbps link for 4.2 Mbps, which is the over-commit this whole change exists to stop. The floor is gone; the measured budget is authoritative, and the room-level `MIN_SCREEN_UPLOAD_BUDGET_BPS` is where a floor belongs. At `SHARE_RATE=1mbit` the picture holds 640x360 to 1280x720 at about 20 fps rather than collapsing. The camera path was untouched here and fixed separately (see the entry above).
- **iOS screen share publishes into LiveKit rooms (2026-09-06):** the share control was hidden on the SFU because the ReplayKit bridge only fed the mesh's `RTCVideoSource`. It now feeds a LiveKit `BufferCapturer` track as well, published as `Track.Source.ScreenShareVideo` on the first frame (the SDK resolves a buffer track's dimensions from what it captures, so a publish before the first frame times out). The bridge itself is unchanged and still NV12 over the App Group socket: the SDK's own broadcast path was rejected because it JPEG-encodes every frame inside the ~50 MB extension process. The publish carries the web's PR #237 ladder, layer for layer, including the large-room cap, so a phone cannot put an uncapped 1080p30 stream in front of a watch party. The control now also follows `welcome.self.canSpeak` on both transports, which is what the web does. **iOS-only, no API change, does not restart `pqp-api`. Device-only and unverified:** ReplayKit broadcast has no simulator equivalent, so the extension, the socket and the rotation mapping are still only exercised by `-pqp.fakeScreenShare`.
- **Character HTTP send (2026-09-04):** `POST /api/channels/:channelId/messages` with `Bearer character:<token>`. Same create + fan-out as the WebSocket `message-create` frame (history, notify, thread replies, outgoing-webhook skip for `is_character`), and the same per-user send bucket, so switching doors buys no extra burst. Clerk sessions get 403. The shared path also refuses a character into any non-server channel, on the socket too; that matches the `dms.ts` rule that a character never opens a conversation. A socketless character is online on that server's member list while an outgoing hook is `active`, and grey when the hook is `failing` or `disabled`. Inert until `CHARACTER_ACCOUNTS_ENABLED=true` on Fly. Caio is still a Clerk user: provisioning mints a **new** account, so he needs a fresh invite and the runner needs the new token. Mint tokens with the existing provision scripts; there is no admin UI. Merging **restarts `pqp-api`**. [`BOT_SEND.md`](./BOT_SEND.md).
- **Outgoing channel webhooks (2026-08-29):** server settings → Integrations. After a human message commits, pqp enqueues a Standard Webhooks-signed `message.created` POST. Skips `is_webhook`, `is_character`, `is_bot`, and per-hook `skip_user_ids` (Caio is a normal user: tick him on the hook). Merging **restarts `pqp-api`**. Incoming webhooks are unchanged. [`OUTGOING_WEBHOOKS.md`](./OUTGOING_WEBHOOKS.md).
- **What's New on the rail, and pinned DMs (staging, 2026-09-03):** Sparkle at the foot of the rail (above Communities) opens the same notes as `/blog`. Lime pip when the newest slug is unseen (`pqp:whats-new-feed`). A one-shot corner card (`novidades-rail`) points at it. Notes are a **weekly catch-up**, not a per-PR duty: do not write one in a feature PR; when Andre asks, follow `.cursor/skills/whats-new/SKILL.md`. Never edit a post already on `main`. Pinned DMs sit under Home, above halls (right-click a conversation: Fixar na barra; cap 15, preference `pinnedConversations`; closing a conversation unpins it). Web + Electron only. Native has neither the sparkle nor the pins. Merging to `main` **restarts `pqp-api`**: `pinnedConversations` is a key in the existing JSONB `settings`, not a new column. Zod on the API strips unknown keys, so a pin is kept in the client and silently dropped until the API image redeploys. The catch-up note claims Baú as shipped: production needs `COMMUNITY_HOME_ENABLED` in the same ship. `main` already has `call-que-se-conserta` for the same day; this branch's note is only Baú, pins, `/android`, and the August leftovers.
- **Voice session resume (web / Electron):** an API restart closes `/ws` with 1001. Web and Electron keep the mesh or LiveKit session and rejoin with the same peer id (HMAC token, hours; 90s orphan TTL in-process). Only a join that sent `resume: true` becomes an orphan; iOS, Android, and old tabs leave as soon as the socket closes. A deploy down longer than 90s cold-rejoins instead of hanging up. Closing a tab sends `leave-voice-room` on `pagehide`, plus a keepalive `POST /api/voice/leave` with the resume HMAC, so a closed tab is not an orphan. Electron turns off background throttling on the main window so a minimized call can still resume. Merging this **restarts `pqp-api`** and drops live voice once (old JS). After a refresh, later API deploys should not cut web/Electron audio.
- **Baú / Community Home (on `main` 2026-09-03, dormant):** Patreon-like durable media feed inside a server, persisted on the API (`community_home_*` tables, `/api/servers/:id/home/*`). Two **server** flags, both default off: `COMMUNITY_HOME_ENABLED` (the surface; off = every route 404s, row hidden) and `COMMUNITY_HOME_VIP_ENABLED` (members-only posts + lock; off = free feed only, no tier chips). Client follows `GET /api/community-home/config`; no `VITE_` flag. On top, each server opts in from Server settings (`servers.community_home_enabled`, default off). Members get a one-time intro card (a preference), managers get an empty-state guide, a composer with preview, drafts and schedule, likes, two-comment teasers. Merged to `main` on 2026-09-03 with **both flags unset**, so the tables exist, every route 404s and no row renders until an operator sets them. Turning it on is `fly secrets set COMMUNITY_HOME_ENABLED=true` plus the per-server opt-in; that restart is the only one it needs. Pricing / owner-set plans analysis in [`BAU_VIP_STRATEGY.md`](./BAU_VIP_STRATEGY.md). See [`COMMUNITY_HOME.md`](./COMMUNITY_HOME.md).
- **Donations page (2026-09-01):** `/apoie` and `/support`, GitHub Sponsors plus optional Pix, with a footer link. Donations only: no tiers, no perks, nothing unlocked. Hosted-only through `VITE_SPONSOR_URL` / `VITE_PIX_KEY` / `VITE_PIX_BRCODE` (`client/src/lib/support-links.ts`); all empty hides the page and the link, so a self-host inherits nothing. `.github/FUNDING.yml` puts the Sponsor button on the repo. Client-only, does not restart `pqp-api`.
- **Copy Discord layout (2026-08-25, bits/icon 2026-08-30):** paste a Guild Template link (`discord.new/…`) when creating a community. Preview names the private channels and what will be dropped, then one transaction creates the pqp server. Roles get a named permission subset (never Administrator), VIEW/SEND/CONNECT overwrites, and the Discord icon when storage is on. Voice bitrate stays dropped. No Discord OAuth, no bot, no members or messages. See [`DISCORD_IMPORT.md`](./DISCORD_IMPORT.md).
- **Staff ladder (2026-08-28, PR #141):** Owner / Admin / Manager / Moderator / `@everyone`, seeded on every server. Authz is permission bits, not compatibility rank. Only the owner person bypasses role hierarchy. Kick and ban are separate bits. Profile card ticks cargos instead of Promote/Demote. Cargos settings has Display / Permissions / Members. Merging this **restarts `pqp-api`** and drops live voice for a moment. iOS Promote/Demote still maps to the Admin cargo. The contact sheet from [#139](https://github.com/rafaelcg/pqp/pull/139) is already on `main`; this PR sits on top of it.
- **In-app profile card (2026-08-28, PR #139):** restyled as a contact sheet. Equal action tiles, rank marks (crown / shield) beside the name, and an about pane (Depoimentos / Contas / Comunidades; Contas first when connections exist). Turma and caça-bugs render as a medal and a pin, not chips. Member list search is in the same pass. Client-only. Cargos ticks land in PR #141.
- **Members dialog (2026-08-28):** same Dono / cargo / Online / Offline groups as the sidebar, with counts. The ⋯ opens the portaled context menu (it flips above the last row instead of clipping). Banidos stays last and lazy.
- **Cargos hint (2026-08-28, PR #141):** one corner card for people who can manage cargos, once per device. CTA opens server settings on Cargos. localhost never records the impression, so a refresh brings the card back. Playwright (`navigator.webdriver`) does not show it.
- **Cargos settings tab (2026-08-28):** inset grouped list + editor (same language as the Members dialog). Drag a cargo to reorder it (ghost preview, grab cursor); Dono stays at the top and `@everyone` at the bottom. Colour is an in-app HSV picker (presets + hex still there; the OS colour panel stays hidden). Staff cargos can reset name, colour, and permissions to the seed. Homemade cargos reset colour by clearing it, and permissions back to none. Members add/remove are short equal-width actions.
- **An Android app exists, and it is native (2026-08-25, branch `android-app`, PR open):** Umami puts the audience at Windows 76%, **Android 14%**, iOS 6%, which is more than twice iOS's reach with no app at all while two iOS builds shipped the same night. A Trusted Web Activity over the existing PWA was costed first and rejected on capability, not on taste: a tab cannot hold a foreground service, so a call would die the moment somebody switched apps, and **no mobile browser implements `getDisplayMedia`**, so a wrapper can never share a screen. KMP has nothing to share with (iOS is plain Swift); React Native would need the two features that matter written in Kotlin anyway. What landed in `android/` is Kotlin + Compose + Material 3: dev-bypass auth (Clerk's `AuthView` wired but keyless here), the age gate, servers, channels with categories, and the full text round trip, **verified against a live local server** with the message persisted under a real id. Voice is built with the mandatory foreground service and was tested with **two emulators in one room**: politeness matched `peer-connection-manager.ts`, exactly one side offered, one side drove the ICE restart, both reached `CONNECTING`. Both then hit `FAILED`, which is pitfall #1 reproduced rather than a bug, because `/api/ice-servers` was serving STUN only (local `.env` has placeholder `VITE_TURN_*`) and two emulators are cross-NAT. That exposed a real one: the call bar said "2 in this call" while nobody could hear anybody, so per-peer media state is now reported and the bar says so. **Do not write "Android voice works" until somebody has heard somebody.** Screen sharing via `MediaProjection` is the biggest remaining win and is not started; push (FCM) is not started; no CI job was added, deliberately. Detail: [`ANDROID.md`](./ANDROID.md).
- **Turma dos 1000 (2026-08-26):** numbered founding mark (`user_badges.ordinal`, slug `turma-1000`) for the first 1,000 human accounts (`NOT is_webhook AND NOT is_character`), ordered by `created_at, id`. Numbers are never recycled. It renders as a struck medal (the number lives on the face), not a text chip, on the public profile and the in-app card. **Nothing is granted until that count is reached.** `insertNewUser` calls `stampTurma1000()` after every new human (advisory lock, one-shot, skipped under Vitest). Until then the achievements array stays empty and the medal hides itself. Manual fallback: `server/scripts/grant-turma-1000.mjs` against `DATABASE_URL` (not in the Fly image). Merging this restarts `pqp-api` and drops live voice.
- **Appearance presets (2026-08-25):** Settings → Appearance picks Classic (default; id `signal`), Harmony, Hearth, or Night. Second axis beside light/dark/system (`data-appearance`, `pqp-appearance`, synced on the account). Night is dark-only: it pins brightness to dark and turns Light / System off. Token skins only; no layout clone and no other product's hex values. High contrast is a third axis (`data-contrast="more"`, `pqp-contrast`, default `system` follows `prefers-contrast: more`). Accent hue is a fourth axis (`data-accent="custom"`, `pqp-accent-hue`, `default` or 0–360). See [`THEMING.md`](./THEMING.md).
...
- **Voice overwrite extras (2026-09-06):** Video (`STREAM`) is its own bit, split from Speak. Mute Members and Move Members resolve against the voice channel, so a cargo can mute in Lobby and not in Staff. The channel editor does not offer those two: they live on Cargos. Existing Speak overwrites were copied onto Stream. Admins get a settings cog on hover in the channel list (Discord's pattern). Rename, private, permissions, and webhooks are one "Channel settings" item, not four menu rows. Owner and Administrator stay locked on in Quem fala: they skip overwrites, same as private channels. `restarts-api`.
...
- **In-app sounds (2026-08-23, demo on `feat/in-app-sounds`):** cinematic UI SFX (CC0): mention = `mention`, voice join/leave = `select`/`deselect`. No ping for ordinary messages. Incoming call rings are synthesised: Classic plus Chime, Pulse, Marimba and Glass in Settings → Notifications. Outgoing stays a quieter dual-tone. OS banners stay silent. Settings → Notifications has a Sounds block. DND still mutes mention. Your own join/leave play on the click, not after mic/welcome; the mic pipeline waits until that cue finishes (or 750ms) so opening capture cannot cut the tail. One sample at a time so join cannot stack on leave or the outgoing ring. Mentions also play in the open channel. Other people joining/leaving the lobby you are in still play.
- **Fix: a screen share no longer dies because the connection was busy (2026-08-25):** reported through the in-app form on 23 Aug as `Failed to execute 'setLocalDescription' on 'RTCPeerConnection': Failed to set local offer sdp: ...`, in English, in a Portuguese app. `setLocalScreenStream` offered unconditionally, but an offer is only legal from a `stable` connection, and a click arrives whenever it arrives: land it during any other exchange (answering a peer's offer, an ICE restart after a blip, the other person turning their camera on) and the rejection escaped the manager, `startScreenShare` caught it and ran `stopScreenShareInternal`. The capture was destroyed by its own timing. Offers are now *requested* rather than commanded (`requestNegotiation`), and one that cannot go out yet waits in the retry loop that already existed for tracks with no m-line; a peer owed an offer is tracked explicitly (`owedOffer`), because a *removal* leaves no sender behind to notice. The offer itself is applied with argument-less `setLocalDescription()`, which closes the second half of the same fault: `setLocalDescription(await createOffer())` is two steps with a gap, and an offer built before that gap describes a session that no longer exists. `setLocalCameraStream` had the same fault and the same fix. Not screen-audio specific, but a share with audio is the heaviest renegotiation the mesh does, which is why it surfaced the day after that shipped.
- **i18n (en + pt-BR):** i18next core, JSON catalogues, lazy Portuguese, Electron menus. See [`I18N.md`](./I18N.md).
- **Unlimited screens on the voice server, and a mesh limit that is a measurement (2026-09-08):** the ask was "unlimited webcams and screens for all channels". On the voice server, yes: `SCREEN_SHARE_LIMIT.livekit` was 4 because Zoom says 4, and a presenter uploads once whatever the room size, so it is `null` now and the box is priced instead (`decideVideoAdmission`, `VOICE_PROMOTION_MAX_SFU_MBPS`), refused as "no room for more screens right now" rather than as a count. **A share is now charged at a share's bitrate**: `estimateRoomMbps` was charging every publication 1.5 Mbit/s, the top of the CAMERA ladder, when a share asks for 4 on an explicit 1080p, so dropping the count without this would have swapped a too-strict count for a too-generous price by nearly three. Shares are charged 4 Mbit/s at or below 20 participants and 1.5 above it, exactly where `screenSimulcastPlan` holds the top layer; shares have published a simulcast ladder since 2026-09-06, so a small tile of a share already gets a small layer. On mesh, no, and 2 shares / 3 cameras were wrong in the other direction: they were guesses about a typical home link applied to every link. `meshVideoLimit` derives the number from the uplink `screen-upload-budget.ts` already measures and the room size, clamped to at least 1 and at most 4 shares / 6 cameras, so three people on fibre can all share and five people on 2 Mbit/s are stopped at one, and reaching it still **promotes**. After the room-size trigger (PR #388, a room moves at four people on its own), the measured mesh limit is consulted only for two and three person calls on the hosted deployment, which are exactly the ones kept peer to peer on purpose, and for every room on a self-host with no voice server. A client that reports nothing (every native client today) gets exactly the old constant. A client that lies gains nothing: the report is clamped to the 1 to 16 Mbit/s window the controller itself runs in, the server takes the narrowest report in the room, and on the voice server no client number is read at all. `stageTileSlots` now holds two slots back for faces so a room of twelve shares still draws your own picture and whoever is talking. `restarts-api`. Not verified on a real phone or on the production box. Detail: [`voice-backends.md`](./voice-backends.md) "Unlimited screens, and a mesh limit that is a measurement".
- **Everyone in a voice-server room can turn a camera on (2026-09-08):** the camera published `simulcast: false`, so there was one copy of a face on the SFU and every viewer received it whatever size their tile was; twenty cameras was twenty full-size streams into every phone, and that is what `CAMERA_LIMIT.livekit = 8` was really protecting. Three changes and the count is gone. **The camera publishes a ladder** (720p / 360p / 180p, `CAMERA_SIMULCAST_RUNGS`), so `adaptiveStream` finally has something to pick from: measured against a local LiveKit, a 160x90 tile went from receiving 1280x720 at 675 kbps and 0.81 ms of decode per frame to 320x180 at 149 kbps and 0.10 ms, and one viewer's total in a 24-camera grid went from 45.8 to 6.0 Mbit/s. **The grid is bounded** at 12 tiles on a laptop and 6 on a phone (`stageTileSlots`), shares and the pin and our own camera always kept, speakers promoted out of the overflow, and everything past it becomes a chip in the strip whose stream the server then stops forwarding. **`CAMERA_LIMIT.livekit` is `null`**: a room already on the SFU asks `decideVideoAdmission`, the same `VOICE_PROMOTION_MAX_SFU_MBPS` budget the promotion guard uses, and is refused with `camera-denied` plus `voice.videoAdmissionRefused` when the box's estimated egress would go over it. `CAMERA_LIMIT.mesh` was still 3 and still promoted on the fourth; it became a measured number the same day, see the row above. The budget prices the worst case (every participant on every publisher's top layer), so a client that lies about its tile size gains nothing. `restarts-api`. Not verified on a real phone or on the production box. Detail: [`voice-backends.md`](./voice-backends.md) "Cameras: the ladder, the bounded grid and the end of the count".
- **A fourth camera moves the call to the voice server instead of being refused (2026-09-08, PR #366):** reported in the QG as *"não dá pra ter mais de 3 câmeras ligadas nessa porra"*. `CAMERA_LIMIT.mesh` is 3 and stays 3, because a mesh camera is a full uplink copy per peer; what changed is the answer. Where LiveKit is configured, a camera (or a share) past the mesh cap **promotes the whole room to the SFU** and turns on, and the cap after the move is the SFU's (4 shares; the camera count was removed entirely on 2026-09-08, see the row above). The pin is rewritten with one conditional UPDATE so two clicks promote once, `voice.transport` moves the seats on the other machine, and each seat that negotiated `voice-transport-changed` swaps its media in place keeping its peer id, mute, camera and share. A seat that did not (**iOS and Android today**) is released with a notice and rejoins onto the SFU rather than being left on a mesh nobody relays. `VOICE_PROMOTION_MAX_SFU_MBPS` (default 600, against a box measured clean at 880 to 935) refuses the promotion when the media server is already loaded, and the old camera limit stands. Nothing changes without `LIVEKIT_*`. `restarts-api`. Detail: [`voice-backends.md`](./voice-backends.md) "The one time a live room changes transport"; the native follow-up is item 14c in [`PARITY.md`](./PARITY.md).
- **More than one person can share a screen (2026-08-23):** the exclusive presenter lock is gone. Caps were **2 concurrent shares on mesh, 4 on LiveKit** (`SCREEN_SHARE_LIMIT`); on 2026-09-08 the LiveKit count went away entirely and the mesh one became a measurement, see the row above. The web layout auto-shows every share (split on a wide window for two; focus + chips otherwise). Screen audio follows hook state: one share is that share, two are both, three-plus is the focused share only. Old clients that still take the first `sharingScreen` roster entry see only that first sharer.
...
- **Operator dashboard (2026-08-30):** `tools/admin-dashboard/` (Cloudflare Worker `pqp-admin`, Basic Auth on the page / `/metrics` / `/health`) renders `GET /api/admin/metrics` plus an Android block the Worker owns itself: APK button clicks (`POST /apk-click` → KV) and GitHub `download_count` on `android-beta` / `pqp.apk`. The API also sends a `product` block (friends, attachments, invites, push). No seed numbers — skeletons until the first real read. See `tools/admin-dashboard/README.md`.
...
| **iOS screen share into a LiveKit room** | **Not verified**. The publish path builds and its ladder is unit-tested (`ScreenSharePlanTests`), but no phone has published a share into an SFU room |
