...
**How many people fit in one voice room, measured (2026-09-07).** On staging sized like production, joins stop fitting the client's own 12 second budget at roughly **300 to 400 people already in the room, centre about 350**, measured three times against shipping `main`. Before the roster delta work it was 225 to 250, so that change bought about 1.4x. What runs out is **bandwidth**, not CPU (never past 36 % of two cores) and not the Postgres pool (saturated four seconds in four minutes, behind the wire rather than ahead of it): voice signalling fan-out is O(room size) per arrival, and `presence-update` is now the largest thing on the wire at 45 kB a frame and about 70 % of the bytes. The number is a floor, because the harness concentrates every simulated client on one link, and it predates the WebSocket compression and audience roster work that was in flight when it was taken. Method, safety rules and how to run it again: [`STAGING.md`](./STAGING.md) under "Load testing staging".
