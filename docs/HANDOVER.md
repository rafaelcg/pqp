...
- **Staff ladder (2026-08-28, PR #139):** Owner / Admin / Manager / Moderator / `@everyone`, seeded on every server. Authz is permission bits, not compatibility rank. Only the owner person bypasses role hierarchy. Kick and ban are separate bits. Profile card ticks cargos instead of Promote/Demote. Cargos settings has Display / Permissions / Members. Merging this **restarts `pqp-api`** and drops live voice for a moment. iOS Promote/Demote still maps to the Admin cargo.
- **In-app profile card (2026-08-28):** restyled as a contact sheet. Equal action tiles, rank marks (crown / shield) beside the name, cargos without `@everyone` or Owner pills, and an about pane (Depoimentos / Contas / Comunidades; Contas first when connections exist). Turma and caça-bugs render as a medal and a pin, not chips. Member list search is in the same pass. The staff-ladder half of this PR is not client-only.
