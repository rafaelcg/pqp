...
- `LIVEKIT_*` set, for capacity rather than correctness. With two live instances a room nobody has pinned yet opens on the SFU when one is configured (`voice.meshGuardForcedSfu` in the log) and on mesh otherwise; a room already pinned on mesh is adopted by whichever machine the join lands on (`voice.meshPinAdopted`), with offer, answer and ICE crossing the bus on `voice.signal`. Nothing is refused for being mesh any more.
