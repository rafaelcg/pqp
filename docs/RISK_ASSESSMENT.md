...
| # | Gap | Consequence |
|---|---|---|
| G1 | **No UI for the instance moderation queue.** The endpoint `GET /api/reports/instance` exists; nothing in `client/src` calls it. Access is gated on `INSTANCE_MODERATOR_CLERK_IDS` — **if that env var is unset, nobody can read the queue at all**. **Fixed 2026-09-14:** `GET /api/reports/all` (a superset — every report, server-scoped or not, named by server) is now surfaced in Settings as **Moderação da instância**, visible only when that route answers 200. The env-var gate itself is unchanged: still nobody without `INSTANCE_MODERATOR_CLERK_IDS` set | Every DM report — grooming, harassment, cyberflashing, CSAM in a DM — used to land somewhere only reachable by `curl`, and only if an env var was set. The read path is fixed; the operator still has to set the env var for anyone to use it |
