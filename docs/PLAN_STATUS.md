...
| 4 Self-host / Fly | Done | Docker Compose + docs; hosted Pages + Fly (`pqp-api`, gru) live. Railway is retired; nothing points at it. Two-machine rolling deploys **prepared, not flipped** (2026-09-14): `fly.toml`/`deploy-api-fly.yml`/`docs/deploy-fly.md` §6a-bis ready; flip gated on prerequisite PRs + a green staging matrix, targeted for a quiet window before the 2026-09-19 party |
...
| Character HTTP send | Done (`POST /api/channels/:id/messages` as `Bearer character:<token>`; same create path and send budget as WS; behind `CHARACTER_ACCOUNTS_ENABLED`. Member list: socketless character is online while an outgoing hook is `active`. [`BOT_SEND.md`](./BOT_SEND.md)) |
