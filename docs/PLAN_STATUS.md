...
| Turma dos 1000 | Code shipped; **not granted until 1,000 human accounts**. Numbered mark (`turma-1000`, `user_badges.ordinal`) for the oldest 1,000 humans by `created_at, id`. Auto-stamps from `insertNewUser`. Chip hidden while the list is empty. Manual fallback `server/scripts/grant-turma-1000.mjs`. |
