...
Written 2026-09-07 from the runs of that afternoon (UTC 18:44 to 19:40). The
...
yet measured, the production 2 vCPU box should carry roughly 300 to 400
viewers at 720p / 1.5 Mbps, or 140 to 150 at an explicit 1080p / 4 Mbps
share; **500 at 720p with margin needs the 4 vCPU plan** (`vhp-4c-8gb-amd`,
about $48 a month list price against the $36 the current box costs). The port
change has **not** been applied to production as of this writing.
...
### Ladder E and ladder F: the ceilings

Not yet run at the time of writing. The ladders replace the extrapolation in
section 5 with a measurement. Shape (`lk-ladder.sh` on generator `d`): our
own presenter publishing in a room, `lk load-test` subscribers stepped in at
200, 300, 400, 500, 600 (no decode, 10 a second), the SFU sampled per step;
`ladder-summarize.mjs` prints one row per step with steady egress, CPU p50 /
p95, receive-buffer drops a second and `lk`'s own loss. Two ladders are
planned so both axes of section 5 are measured: box size (the 4 vCPU test box
against production's 2 vCPU plan) and bits per viewer (720p at 1.5 Mbps
against an explicit 1080p at 4 Mbps). When they land, record for each: the
box, the profile, the per-step table, and the first step where drops appear
or `lk` loss passes 1%. That step is the ceiling; the step before it is the
number to plan against.

| subscribers | egress Mbit/s (steady p50 / peak) | SFU CPU p50 / p95 | rcvbuf drops per s (mean / peak) | lk loss (median / max) | note |
|---|---|---|---|---|---|
| (fill in) | | | | | |

## 5. Extrapolation, stated as such

Everything in this section is arithmetic on A2 and will be replaced by the
ladders. Basis: **A2 carried 468 receivers at 837 Mbit/s and 69% of
4 vCPU** on the hold's median second. That is 1.79 Mbit/s of SFU egress per
viewer for a 1.35 Mbit/s share (RTP, UDP and IP headers, RTCP and audio on
top), and about 5.9 thousandths of a core per viewer.

| Box | Share profile | Extrapolated viewers | How |
|---|---|---|---|
| 2 vCPU (`vhp-2c-4gb-amd`, production) after the port change | 720p at 1.5 Mbps (the large-room default) | **about 300 to 400** | half of A2's linear wall of about 680 at 100% CPU, with the low end leaving working margin |
| 2 vCPU after the port change | explicit 1080p at 4 Mbps top layer, every viewer on it | **about 140 to 150** | the 720p figure divided by 2.67 (4 over 1.5) |
| 4 vCPU (`vhp-4c-8gb-amd`) | 720p at 1.5 Mbps | **about 700** | A2's 468 scaled to 100% CPU; 500 sits at about 74%, which is the margin the summary means |
| 4 vCPU | explicit 1080p at 4 Mbps | **about 280** | 700 divided by 2.67 |

Assumptions, each of which the ladders test:

1. **CPU is linear in forwarded bytes and viewers.** True over the range A2
   and C0 span on this box (16% at 168 Mbit/s, 69% at 837 Mbit/s: about
   0.095 and 0.082 points per Mbit/s, a small fixed cost and then a
   straight line), unverified above 959 Mbit/s.
...
3. **The NIC and the plan's port speed carry it.** The test box peaked at
   959 Mbit/s with zero NIC drops; nothing above that has been observed.
   280 viewers at 1080p is about 1.5 Gbit/s. Check the plan's port speed
   before promising a 1080p room of that size.
...
5. **The single-viewer overhead factor holds** (1.79 over 1.35 in A2). Loss
   on real last miles adds retransmissions that a clean datacenter does not.

Derived, for the egress budget: a three-hour party of 500 viewers at A2's
...
1. **Cores.** Linear, and the only lever that is a reboot: the 4 vCPU plan is
   about twice the viewers of the 2 vCPU plan at the same bitrate (A2 against
   the section 5 arithmetic), for about $48 a month list against $36.
...
- **Orchestration**: the run scripts of 2026-09-07 (`run500.sh`, the
  per-host samplers, `summarize.mjs`, `lk-ladder.sh`,
