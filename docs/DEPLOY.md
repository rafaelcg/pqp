...
| PR size | `.github/workflows/pr-size-label.yml` | PR open/sync, `workflow_dispatch` | Labels the PR `size/XS`–`size/XL` from the diff (lockfiles excluded), and `restarts-api` when the diff touches `server/` or `packages/shared`. Review signal, not a merge gate. |
