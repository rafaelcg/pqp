...
| PR size | `.github/workflows/pr-size-label.yml` | PR open/sync, `workflow_dispatch` | Labels the PR `size/XS`–`size/XL` from the diff (lockfiles excluded). Review signal, not a merge gate. |
