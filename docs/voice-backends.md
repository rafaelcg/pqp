...
- **Mesh spans instances (since 2026-09-08); a fresh room still prefers the SFU.** The mesh guard in `join-voice-room` counts the other live leases in `voice_instances`. When there is at least one, it first reads the room's pin: a room already pinned (on either transport) is adopted by whichever machine the join or resume lands on (`voice.meshPinAdopted` when it is mesh), and only a room nobody has pinned is decided by the guard, which sends it to LiveKit when configured (`voice.meshGuardForcedSfu`) and lets it open on mesh otherwise. A signaling frame for a peer this instance does not hold crosses on `voice.signal`, stamped with the sender's room; the instance holding the target applies the same-room rule against its own map before delivering, so the bus is no wider a door than the local relay. The ceiling counts the rows too. A machine that is draining withdraws its lease before it closes a single socket, so it stops counting at once. Until 2026-09-08 the guard refused a join into a room pinned elsewhere (`voice-join-refused`, `reason: "mesh-multi-instance"`), which hung up four resumes in the 2026-09-07 window; that refusal, and the `voice.meshClusterUnsafe` warning that went with it, no longer exist.
