...
- **The ladder:** `LIVE_HLS_LADDER`, a comma-separated list of rungs, default `1080p60,720p30,480p30`. One egress encodes one profile (`TrackCompositeEgressRequest` carries repeated outputs but a single `options` oneof, protocol 1.50.4 / egress v1.14.1), so a ladder is one egress per rung plus a master playlist listing them. Rungs are `1080p60` (8000 kbit/s), `1080p30` (6500), `720p30` (3200), `720p60` (5500), `480p30` (1200) and `360p30` (700), each optionally overriding its bitrate (`1080p30@3500`). `1080p60` is the default top so a 24 fps film on a 60 Hz display is captured and transcoded at the host's display cadence instead of recadencing to 30 (which is what made that film judder). Do not stack `720p30` and `720p60`, or `1080p30` and `1080p60` (the viewer's pin is by height). A game party that should not spend a 1080p60 encode wants `720p60,480p30`. The bitrates keep a ≥2.5× gap so hls.js does not oscillate. `LIVE_HLS_PRESET` still names a one-rung ladder. Anything unreadable logs `voice.hlsLadderInvalid` once per value and is skipped; a list with nothing valid in it falls back to the default.
