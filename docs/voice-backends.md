...
A watch-party screen share can also go out as HLS: a LiveKit Track Composite egress (`server/src/voice/hls-egress.ts`) writes 2 s segments to the dedicated `LIVE_HLS_S3_*` bucket and viewers get the playlist through the signed proxy (`GET /api/voice/hls-playlist/:channelId/:startedAt`, `server/src/voice/hls-playlist-proxy.ts`). The transcode only exists on LiveKit, so a **`watch_party`** channel with live HLS on is pinned to `livekit` (`reason=hls`) even in a two-person server, ahead of the member-count rows above; the channel override still wins. Only that channel type: an ordinary voice channel is decided by the member-count rows exactly as it is with the flag off (`liveHlsForcesSfu` in `transport-policy.ts`), and `pickHlsSharer` refuses to feed a transcode from anywhere but a `watch_party` room. That pair is why `LIVE_HLS_ENABLED` can be set instance-wide without moving unrelated calls onto the media box.

Signed mode (`LIVE_HLS_SIGNED_URLS` unset or `true`) is what production runs: the `pqp-live` bucket is private, the proxy rewrites segment lines into presigned URLs, and the API's own reads (the readiness probe after egress starts, the proxy's playlist fetch) are presigned endpoint-form GETs (`internalPlaylistUrl`). `LIVE_HLS_PUBLIC_BASE_URL` is therefore not part of "configured" in signed mode; it is required only with `LIVE_HLS_SIGNED_URLS=false`, where it is the raw URL a viewer is handed.
...
- **The ladder:** `LIVE_HLS_LADDER`, a comma-separated list of rungs, default `720p30`. One egress encodes one profile (`TrackCompositeEgressRequest` carries repeated outputs but a single `options` oneof, protocol 1.50.4 / egress v1.14.1), so a ladder is one egress per rung plus a master playlist listing them. Rungs are `1080p60` (8000 kbit/s), `1080p30` (6500), `720p30` (3200), `720p60` (5500), `480p30` (1200) and `360p30` (700), each optionally overriding its bitrate (`1080p30@3500`). 720p30 is the supported watch-party ladder: 1080p30 tiled HLS hit RTP gaps and egress CPU on the media box. Named 1080 and 60 fps rungs stay available (`1080p30,720p30`, or `1080p60,720p60@3200,480p30` so a 24 fps film on a 60 Hz display is captured and transcoded at the host's display cadence). Do not stack `720p30` and `720p60`, or `1080p30` and `1080p60` (the viewer's pin is by height). `LIVE_HLS_PRESET` still names a one-rung ladder. Anything unreadable logs `voice.hlsLadderInvalid` once per value and is skipped; a list with nothing valid in it falls back to the default.
- **The master playlist:** `GET /api/voice/hls-playlist/:channelId/:startedAt` now answers the master (one `EXT-X-STREAM-INF` per rendition that actually started, with `BANDWIDTH`, `AVERAGE-BANDWIDTH`, `RESOLUTION`, `FRAME-RATE` and `CODECS`, lowest first), and `.../:rung` answers one rendition's media playlist with its segment lines presigned as before. Variant URIs are root-relative and carry the viewer's own `?t=` token, because relative resolution drops the master's query string but keeps the variant's, which is the only way a header-less player authorises the second request. A session with no rung rows predates the ladder and still gets its single media playlist.
- **The session cap:** `LIVE_HLS_MAX_SESSIONS`, default 3. The ladder budget below degrades a party and by design never refuses one (the lowest rung always starts), so this is the only thing bounding concurrent transcodes on the media box. The party past the cap gets no HLS, logs `voice.hlsSessionsCapped`, and its WebRTC share still reaches everyone seated in the room. `liveHls.sessions` / `liveHls.maxSessions` on `/api/admin/metrics`.
- **The budget:** `LIVE_HLS_MAX_LADDER_MBPS`, default 450. A rendition costs roughly one core on moving content and the box is 4 vCPU, so it is priced at a quarter of `VOICE_PROMOTION_MAX_SFU_MBPS` (150 Mbit/s) and checked against both that ladder budget and the promotion budget with the WebRTC already on the box (`estimateSfuLoadMbps`), so the ladder and the cameras cannot each spend the same core. The **lowest rung always starts**: the budget governs the extra renditions, never whether the stream exists. Refusals log `voice.hlsRungRefused` and appear in `voice.hlsStarted`'s `refused` list. A secondary rung that dies mid-stream is dropped from the master and the rest keeps playing (`voice.hlsRungDied`); only the primary dying restarts the session.
- **The presenter's own picture:** the egress transcodes from the published WebRTC track, so a 720p source cannot produce a 1080p rendition. While a session is live on the channel and the presenter is sharing on the SFU, `screenSimulcastPlan` raises the published top layer to the ladder's top (`stream.topHeight`, post-budget) if the measured uplink can carry it (`availableOutgoingBitrate` off the selected candidate pair, 4 Mbit/s plus 25% headroom; unmeasured allows it). The large-room cap is untouched for an ordinary call with no egress, and the presenter's own smaller pick is never overruled.
- **The viewer's control:** the player lists the available rungs plus Auto, remembered per browser as a HEIGHT rather than a level index (`client/src/lib/hls-quality.ts`), so a pin survives a restarted egress and falls back to Auto rather than to a neighbour when its rung is gone.
...
- **Config route:** `GET /api/live-hls/config` returns `{ enabled, delaySeconds, allowlisted, ladder }`; `enabled` is per server when `?serverId=` is given and the global flag otherwise, `allowlisted` says whether a list exists at all.
...
- **The signal is the server's people, not the first joiner.** Each account's last seen country (`users.last_country` / `last_country_at`: the two-letter `CF-IPCountry` of its WebSocket upgrade, never an IP) is written at WS auth, at most every 6 h unless it changed, and only with regions on (`server/src/voice/region-audience.ts`). When a room opens, the channel's server is tallied: members seen in the last 30 days, bots and characters excluded, each country mapped through `LIVEKIT_REGION_COUNTRIES` (unmapped counts for the default region), cached per server for 3 h. At least 5 known members and one region holding at least 60% of them: that region (`reason=server-majority`). Enough members but no such majority: `LIVEKIT_REGION_DEFAULT`, else home (`reason=server-mixed`), because a mixed call is fine on home (2026-09-24, UK to Sao Paulo: ~193 ms RTT via sao, ~199 via lhr, ~228 via mia). Fewer than 5 known members (a new server, or the first days after this shipped): the first joiner's `CF-IPCountry` as before (`reason=country` / `default`). The decision order is `decideSfuRegion`: conversations and watch party channels stay home (and, only with the rollback `LIVEKIT_REGION_REQUIRE_CAP=true`, first joiners without the `sfu-region` capability; by default every client, phones included, is trusted to dial the URL it is handed, audited in `docs/plans/SFU_REGIONS.md` §Clients); then the operator override (`channels.sfu_region`, dashboard or `PUT /api/admin/channel-sfu-region`); then the server's people; then the first joiner's country. `voice.regionPinned` carries `share`, `sample` and `byRegion` for the two server reasons.
...
With `LIVEKIT_REGIONS` unset, nothing above runs: no header is read for routing, no column written, no field added. `server/src/ws/voice-sfu-region.test.ts` pins the server-majority rule (a visitor cannot move a Brazilian server's room, a split server stays home, too few members falls back to the first joiner), pinning, the second replica, resume across a restart (row and token), the capless-phone default and its rollback switch, the watch-party rule and the flag-off guarantee, on a real Postgres with `VOICE_REGISTRY=postgres` and `CLUSTER_BUS=postgres`; `server/src/voice/region-audience.test.ts` pins the recording and the tally.
...
- **Scale.** The `--dev` verification above was two to twelve participants on localhost. Since then the self-hosted box has been load-tested at much larger sizes on staging and has carried a real watch party of over a hundred people (2026-09-05), which is the one capacity figure this project states publicly. The method for those load tests is in [`docs/CAPACITY.md`](./CAPACITY.md); the measured results are held in the operator's copy at `~/.config/pqp/capacity-measured.md`, and that document says why. Still no claim about cross-NAT paths or TURN interaction on the SFU path.
