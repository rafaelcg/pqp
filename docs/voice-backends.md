...
- **The ladder:** `LIVE_HLS_LADDER`, a comma-separated list of rungs, default `1080p30,720p30,480p30`. One egress encodes one profile (`TrackCompositeEgressRequest` carries repeated outputs but a single `options` oneof, protocol 1.50.4 / egress v1.14.1), so a ladder is one egress per rung plus a master playlist listing them. Rungs are `1080p30` (4500 kbit/s), `720p30` (1800), `720p60` (3200), `480p30` (900) and `360p30` (500), each optionally overriding its bitrate (`1080p30@3500`). `720p60` is opt-in: it is not in the default, capture/publish stay at 30 fps unless it is listed, and it costs roughly 1.6–2× a 720p30 encode on the media box. Do not stack `720p30` and `720p60` (the viewer's pin is by height). A game party wants `720p60,480p30` more than `1080p30,720p60`, which is a tight ABR gap. The bitrates are deliberately not LiveKit's presets: those are 3000 and 4500, a gap of 1.5x, which leaves hls.js only about 1.10x between "switch up" and "switch down" and oscillates. 1800 against 4500 is 2.5x, and makes 720p a real mobile-data rung. `LIVE_HLS_PRESET` still names a one-rung ladder. Anything unreadable logs `voice.hlsLadderInvalid` once per value and is skipped; a list with nothing valid in it falls back to the default.
